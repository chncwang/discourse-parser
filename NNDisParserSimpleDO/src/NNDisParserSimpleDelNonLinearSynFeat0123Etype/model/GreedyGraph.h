	...
	inline void decode(Graph *cg, const Instance &inst, const vector<CAction> *gold_actions = NULL) {
		...
		while (true) {
			...
			for (int idx = 0; idx < candidate_action_num; idx++) {
				...
				if (scored_action.state->_is_gold && scored_action.ac == gold_answer) {
				...
				if (cg->train && scored_action.ac != gold_answer) pGenerator->_next_action_score.outputs[idx].val[0] += pOpts->delta;
			...
			if (cg->train && !gold_action_scored) {
				...
				cout << ", step:" << step;
				cout << ", action: " << gold_answer.str(*pOpts) << endl;
			}
			...
			if (cg->train) {
				...
				for (int idx = 0; idx < offset; idx++) {
					...
					if (current_scored_action.is_gold) {
						...
						states[step]._is_gold = current_scored_action.is_gold;
						find_next = true;
						break;
					}
				}
				...
				states[step]._is_gold = highest_score_action.is_gold;
			}
