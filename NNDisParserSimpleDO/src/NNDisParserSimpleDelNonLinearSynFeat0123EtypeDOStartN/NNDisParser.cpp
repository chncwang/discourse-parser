...
void DisParser::getGoldActions(vector<Instance> &vecInsts) {
	...
	for (int idx = 0; idx < inst_size; idx++) {
		...
		inst.evaluate(result, span, nuclear, relation);
		if (!span.bIdentical() || !nuclear.bIdentical() || !relation.bIdentical()) {
...
void DisParser::train(const string &trainFile, const string &devFile, 
	...
	for (int iter = 0; iter < m_options.maxIter; ++iter) {
		...
		Metric dev_span, dev_nuclear, dev_relation;
		Metric test_span, test_nuclear, test_relation;
		...
		if (devNum > 0) {
			...
			for (int idx = 0; idx < devNum; idx++) {
				devInsts[idx].evaluate(decodeInstResults[idx], dev_span, dev_nuclear, dev_relation);
			}
			...
			cout << "R: ";
			dev_relation.print();
			if (!m_options.outBest.empty() && dev_nuclear.getAccuracy() > bestFmeasure) {
			...
			if (testNum > 0) {
				...
				for (int idx = 0; idx < testInsts.size(); idx++) {
					testInsts[idx].evaluate(decodeInstResults[idx], test_span, test_nuclear, test_relation);
				}
			...
			if (m_options.saveIntermediate && dev_relation.getAccuracy() > bestFmeasure) {
				std::cout << "Exceeds best previous performance of " << bestFmeasure << ". Saving model file.." << std::endl;
				bestFmeasure = dev_relation.getAccuracy();
...
void DisParser::getSynFeats(vector<Instance> &vecInsts, const string &folder) {
	...
	string file5 = folder + path_separator + "recurs.600.hvec"; 
	...
	while (1) {
		...
		for (int i = 0; i < cur_word_size; i++) {
			...
			assert(vecInfo.size() == 601);
			extern_nodes[node_count].init(300, -1);
			...
			for (int j = 0; j < 300; j++) {
				extern_nodes[node_count].val[j] = atof(vecInfo[j + 301].c_str());
			}
			node_count++;
		}
